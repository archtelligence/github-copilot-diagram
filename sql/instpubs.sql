...
GO
set nocount on
set dateformat mdy USE master
declare @dttm varchar(55)
select @dttm = convert(varchar, getdate(), 113) raiserror('Beginning InstPubs.SQL at %s ....', 1, 1, @dttm) with nowait
GO if exists (
      select *
      from sysdatabases
      where name = 'pubs'
   ) begin raiserror('Dropping existing pubs database ....', 0, 1) DROP database pubs
end
GO CHECKPOINT
go raiserror('Creating pubs database....', 0, 1)
go
   /*
...
GO CHECKPOINT
GO USE pubs
GO if db_name() <> 'pubs' raiserror(
      'Error in InstPubs.SQL, ''USE pubs'' failed!  Killing the SPID now.',
      22,
      127
   ) with log
GO if CAST(SERVERPROPERTY('ProductMajorVersion') AS INT) < 12 BEGIN exec sp_dboption 'pubs',
   'trunc. log on chkpt.',
   'true' exec sp_dboption 'pubs',
   'select into/bulkcopy',
   'true'
END
ELSE ALTER DATABASE [pubs]
SET RECOVERY SIMPLE WITH NO_WAIT
GO execute sp_addtype id,
   'varchar(11)',
   'NOT NULL' execute sp_addtype tid,
   'varchar(6)',
   'NOT NULL' execute sp_addtype empid,
   'char(9)',
   'NOT NULL' raiserror('Now at the create table section ....', 0, 1)
...
GO CREATE TABLE publishers (
      pub_id char(4) NOT NULL CONSTRAINT UPKCL_pubind PRIMARY KEY CLUSTERED CHECK (
         pub_id in ('1389', '0736', '0877', '1622', '1756')
...
GO CREATE TABLE employee (
      emp_id empid CONSTRAINT PK_emp_id PRIMARY KEY NONCLUSTERED CONSTRAINT CK_emp_id CHECK (
...
GO raiserror('Now at the create trigger section ...', 0, 1)
GO CREATE TRIGGER employee_insupd ON employee FOR
insert,
   UPDATE AS --Get the range of level for this job type from the jobs table.
...
where e.emp_id = i.emp_id
   AND i.job_id = j.job_id IF (@job_id = 1)
   and (@emp_lvl <> 10) begin raiserror (
...
ELSE IF NOT (
   @emp_lvl BETWEEN @min_lvl AND @max_lvl
) begin raiserror (
...
GO raiserror('Now at the inserts to authors ....', 0, 1)
GO
insert authors
values(
      '409-56-7008',
...
GO raiserror('Now at the inserts to publishers ....', 0, 1)
GO
insert publishers
...
GO raiserror('Now at the inserts to pub_info ....', 0, 1)
GO
insert pub_info
values(
      '0736',
...
GO raiserror('Now at the inserts to titles ....', 0, 1)
GO
insert titles
values (
      'PC8888',
...
GO raiserror('Now at the inserts to titleauthor ....', 0, 1)
GO
insert titleauthor
...
GO raiserror('Now at the inserts to stores ....', 0, 1)
GO
insert stores
values(
      '7066',
...
GO raiserror('Now at the inserts to sales ....', 0, 1)
GO
insert sales
values(
      '7066',
...
GO raiserror('Now at the inserts to roysched ....', 0, 1)
GO
insert roysched
...
GO raiserror('Now at the inserts to discounts ....', 0, 1)
GO
insert discounts
...
GO raiserror('Now at the inserts to jobs ....', 0, 1)
GO
insert jobs
values ('New Hire - Job not specified', 10, 10)
insert jobs
values ('Chief Executive Officer', 200, 250)
insert jobs
values ('Business Operations Manager', 175, 225)
insert jobs
values ('Chief Financial Officier', 175, 250)
insert jobs
values ('Publisher', 150, 250)
insert jobs
values ('Managing Editor', 140, 225)
insert jobs
values ('Marketing Manager', 120, 200)
insert jobs
values ('Public Relations Manager', 100, 175)
insert jobs
values ('Acquisitions Manager', 75, 175)
insert jobs
values ('Productions Manager', 75, 165)
insert jobs
values ('Operations Manager', 75, 150)
insert jobs
values ('Editor', 25, 100)
insert jobs
values ('Sales Representative', 25, 100)
insert jobs
values ('Designer', 25, 100)
GO raiserror('Now at the inserts to employee ....', 0, 1)
GO
insert employee
values (
      'PTC11962M',
...
GO raiserror('Now at the create index section ....', 0, 1) with nowait
GO CREATE CLUSTERED INDEX employee_ind ON employee(lname, fname, minit)
GO CREATE NONCLUSTERED INDEX aunmind ON authors (au_lname, au_fname)
GO CREATE NONCLUSTERED INDEX titleidind ON sales (title_id)
GO CREATE NONCLUSTERED INDEX titleind ON titles (title)
GO CREATE NONCLUSTERED INDEX auidind ON titleauthor (au_id)
GO CREATE NONCLUSTERED INDEX titleidind ON titleauthor (title_id)
GO CREATE NONCLUSTERED INDEX titleidind ON roysched (title_id)
GO raiserror('Now at the create view section ....', 0, 1)
GO CREATE VIEW titleview AS
select title,
   au_ord,
   au_lname,
   price,
...
GO raiserror('Now at the create procedure section ....', 0, 1)
GO CREATE PROCEDURE byroyalty @percentage int AS
select au_id
from titleauthor
where titleauthor.royaltyper = @percentage
GO CREATE PROCEDURE reptq1 AS
select case
...
group by pub_id,
   type with rollup
GO CREATE PROCEDURE reptq3 @lolimit money,
   @hilimit money,
   @type char(12) AS
select case
      when grouping(pub_id) = 1 then 'ALL'
...
GO
declare @dttm varchar(55)
select @dttm = convert(varchar, getdate(), 113) raiserror('Ending InstPubs.SQL at %s ....', 1, 1, @dttm) with nowait
GO -- -